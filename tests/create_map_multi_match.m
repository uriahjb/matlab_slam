...
for ind = 1000:length(lidar.ts)-100
    ...
    % Draw trajectory
    traj = [traj; state];
    set(trajh, 'XData', traj(:,1), 'YData', traj(:,1) );
    ...
    % Accumulate lidar hits
    lidar_hits_arr = [lidar_hits_arr lidar_hits];
     
    if ind > 50 && mod(ind, 5) == 0

        % Perform local theta search 
        dthetas = -0.1:0.005:0.1;
        vals = [];
        dstates = [];
        ...
        for th = 1:length(dthetas)
            ...
            tf = tforms(state_search);
            lidar_hits_search = tf*lidar_hits_arr;

            
            %lidar_hits_search = tform_scan( state_search, lidar.ranges(:,ind), lidar_norm);
            lidar_corr = [lidar_hits_search(1,:); lidar_hits_search(2,:); zeros(1,length(lidar_hits_arr))];
